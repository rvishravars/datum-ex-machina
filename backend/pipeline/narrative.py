...
def map_to_storyboard(
    stats: Dict[str, Any], tier: str, title: str, unit: str, story: BaseStory = None
) -> Dict[str, Any]:
    """
    ...
    for i, pt in enumerate(sampled_points):
        beat = _classify_beat(
            pt, i, len(sampled_points), trend["direction"], max_change_idx
        )
        panel = _build_panel(
            panel_index=i,
            data_point=pt,
...
def _sample_points(points: List[Dict], n: int) -> List[Dict]:
    ...
    if total <= n:
        return points
    indices = (
        [0] + [round(i * (total - 1) / (n - 1)) for i in range(1, n - 1)] + [total - 1]
    )
...
def _build_panel(
    panel_index, data_point, beat, stats, tier, unit, ci_bands, story: BaseStory
) -> Dict:
    trend = stats["trend"]

    # Who speaks in this panel? (Derived from characters defined in the story)
    characters_present = _select_characters_for_beat(
        beat, data_point, tier, story, stats
    )
    ...
    stats_chart = render_stats_chart_base64(
        ...
        unit=unit,
        ys2=np.array([p["y2"] for p in stats["points"]])
        if all("y2" in p and p["y2"] is not None for p in stats["points"])
        else None,
    )
...
def _build_micro_chart(data_point, stats) -> Dict:
    return {
        ...
        "percent_of_max": round(data_point["y"] / stats["summary"]["max"] * 100, 1)
        if stats["summary"]["max"] != 0
        else 0,
...
def _get_visual_cues(beat, data_point, tier, ci_info) -> List[str]:
    ...
    if data_point["is_outlier"]:
        cues.append("spotlight")
    if beat == "turning_point":
        cues.append("dramatic_lighting")
    if beat in ("tension", "plateau"):
        cues.append("horizontal_lines")
    if beat == "cliffhanger":
        cues.append("edge_fade")
    if tier == "R" and ci_info:
        cues.append("fog_overlay")
    return cues


def _select_characters_for_beat(
    beat: str, data_point: Dict, tier: str, story: BaseStory, stats: Dict
) -> List[str]:
